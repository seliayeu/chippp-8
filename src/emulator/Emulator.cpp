...
void Emulator::executeInstruction(std::unique_ptr<Instruction> inst) {
    ...
    if (byte0 == 0x00 && byte1 == 0xE0) {
        ...
        // might be good to throw something here
    }
...
void Emulator::execute() {
    ...
    while (env.pc < env.end && !WindowShouldClose()) {
        ...
        if (currentTimerTime - previousTimerTime >= 1.0 / 60.0) { // doesn't work well with waitkey
        ...
        executeInstruction(std::move(inst));
