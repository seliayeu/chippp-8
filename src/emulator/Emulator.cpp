...
//     const int clockRate{ 60 };
//
//     const int width{ 64 * 8 };
//     const int height{ 32 * 8 };
//
//     double previousInstructionTime{ GetTime() }
//     double currentInstructionTime{ 0.0 };
//
//     double previousTimerTime{ GetTime() };
//     double currentTimerTime{ 0.0 };
//
//     InitWindow(width, height, "CHIP-8");
//     InitAudioDevice();
//
//     Sound bell{ LoadSound("../assets/bell.wav") };
//
//     while (env.pc < env.end && !WindowShouldClose()) {
//         // update timer registers
//         if (previousTimerTime - currentTimerTime >= (1.0 / static_cast<double>(60))) {
//             currentTimerTime = GetTime();
...
//         EndDrawing();
//
//         auto updateDrawTime{ currentInstructionTime - previousInstructionTime };
//         auto waitTime = (1.0 / static_cast<double>(clockRate)) - updateDrawTime;
//         if (waitTime > 0.0) {
//             WaitTime(static_cast<float>(waitTime));
//             currentInstructionTime = GetTime();
//         }
//
//         previousInstructionTime = currentInstructionTime;
//
//     }
//
//     CloseWindow();
...
void Emulator::execute() {
    ...
    while (env.pc < env.end && !WindowShouldClose()) {
        ...
        if (currentTimerTime - previousTimerTime >= 1.0 / 60.0) {
            if (env.timerRegister > 0)
                env.timerRegister -= 1;
            if (env.soundRegister > 0) {
                env.soundRegister -= 1;
        ...
        currentInstructionTime = GetTime();
        auto updateInstructionTime = currentInstructionTime - previousInstructionTime;
        auto waitTime{ 1.0 / static_cast<double>(clockRate) - updateInstructionTime };
